...
client.py
---------
Main client-side controller for the encrypted P2P chat application.

Responsibilities:
- Initialize GUI, networking, RSA keys, and background threads.
- Coordinate message sending, file sending, and peer updates.
- Connect GUI actions to network operations.
- Manage cleanup and thread termination on exit.
"""
...
class ChatClient:
    ...
    ChatClient
    ----------
    High-level application controller that binds together:
    - RSA key generation/loading
    - GUI initialization
    - Network manager setup
    - TCP/UDP listener threads
    - Server communication thread
    - Message & file send handlers

    This class is the entry point for running a fully functional
    encrypted chat client with peer-to-peer communication.
    """
    def __init__(self):
        """Initialize GUI, RSA keys, network manager, and start all threads."""
    ...
    def handle_send_message(self):
        """Send a text message to the currently selected peer."""
        ...
        # Send TCP message through NetworkManager
    ...
    def handle_send_file(self):
        """
        Handle selecting a file and initiating a UDP file transfer thread.
        """
        if not self.gui.current_peer:
            QMessageBox.warning(self.gui, "Error", "Please select a peer first.")
            return
        ...
        # Preview clickable file message in GUI
        ...
        def on_transfer_complete():
            """Cleanup after a file transfer completes."""
    ...
    def on_peer_disconnected(self, peer_username):
        """
        Handle when the currently-selected peer disconnects
